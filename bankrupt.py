...
#    encoding_prsnbankruptsId = start_url + prsnbankruptsId + middle_url + end_url # Это от старой версии 
    return start_url + prsnbankruptsId + middle_url + end_url


def get_prsnbankruptsId():
# "Это для тестирования по ручному вводу"
...
def check_debtors(debtors):
    for debtor in track(debtors, description="Проверка..."): 
        ...
        asleep = random.randint(500, 2500) / 1000
        time.sleep(asleep)
...
Добавить полосу прогресса проверки
"""